...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} Listing
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.listing.update", [$listing->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('latitude') ? 'is-invalid' : '' }}" type="text" name="latitude" id="latitude" value="{{ old('latitude', $listing->latitude) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('longitude') ? 'is-invalid' : '' }}" type="longitude" name="longitude" id="longitude" value="{{ old('latitude', $listing->latitude) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    ...
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ $id == $listing->user_id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
